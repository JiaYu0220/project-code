import { userId, isLogin, headers } from '../../utils/config';
import {
  getCartLength,
  renderCartNum,
  handleLoginModal,
...
async function message() {
  if (hasCoupons) {
    showToastSwal('將課程加入購物車');
  } else {
    await showToastSwal('將課程加入購物車');
    showToastSwal('首次加入購物車，獲得「體驗課優惠券」、「多堂折價優惠券」');
  }
}
...
async function addCourseToMyCarts(quantity) {
  try {
    ...
    await axios.post(`${_url}/myCarts`, postData, headers);
...
async function addQuantityToMyCarts(myCarts, addQuantity) {
  try {
    ...
    await axios.patch(`${_url}/myCarts/${id}`, patchData, headers);
  } catch (error) {
    showAlertSwal('發生錯誤，請稍後再試');
  }
}
...
async function addCoupon(coupon) {
  try {
    ...
    await axios.post(`${_url}/myCoupons`, postData, headers);
  } catch (error) {
    showAlertSwal('發生錯誤，請稍後再試');
  }
}
